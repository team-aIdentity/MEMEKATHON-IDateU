import svgPaths from "./svg-7k6cdabs0p";
...
function UserAvatar() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative rounded-[24px] shrink-0 size-[48px]" data-name="User Avatar">
      <p className="font-['Inter:Bold','Noto_Sans_KR:Bold',sans-serif] font-bold leading-[21.6px] not-italic relative shrink-0 text-[18px] text-nowrap text-white whitespace-pre">혜</p>
    </div>
  );
}
...
function MessageRow() {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full" data-name="Message Row">
      <p className="basis-0 font-['Inter:Regular','Noto_Sans_KR:Regular',sans-serif] font-normal grow leading-[16.8px] min-h-px min-w-px not-italic relative shrink-0 text-[#6c757d] text-[14px]">상대방이 좋아요를 보냈습니다.</p>
      <Unread />
    </div>
  );
}
    ...
    <div className="bg-[#f8f9fa] content-stretch flex flex-col items-start relative size-full" data-name="Dating Chat Main">
      ...
      <div className="absolute h-[30px] left-[90px] top-[20px] w-[60px]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 60 30">
          <path d={svgPaths.p752d180} fill="var(--fill-0, white)" id="Rectangle 29" stroke="var(--stroke-0, #1976D2)" strokeWidth="1.5" />
        </svg>
      </div>
      <div className="absolute bg-[#1976d2] h-[30px] left-[160px] rounded-[10px] top-[20px] w-[60px]">
      ...
      <p className="absolute font-['Inter:Semi_Bold','Noto_Sans_KR:Bold',sans-serif] font-semibold leading-[13.2px] left-[166px] not-italic text-[12px] text-nowrap text-white top-[28px] whitespace-pre">{`받은 요청 `}</p>
    </div>
  );
}